...
class AnketaModal(ui.Modal):
    ...
    async def on_submit(self, interaction: disnake.ModalInteraction):
        ...
        submit_channel = interaction.bot.get_channel(SUBMIT_CHANNEL_ID)
        if not submit_channel:
            await interaction.response.send_message("Ошибка: канал для заявок не настроен. Сообщите администрации.", ephemeral=True)
            return
        embed = disnake.Embed(title="Новая заявка на вступление", color=0xffb347, timestamp=disnake.utils.utcnow())
...
class AnketaCog(commands.Cog):
    ...
    async def setup_anketa(self, inter: disnake.ApplicationCommandInteraction):
        ...
        if not channel:
            await inter.response.send_message("Не найден канал '📝│анкета'. Создайте его и повторите команду.", ephemeral=True)
            return
        embed = disnake.Embed(title="Добро пожаловать!", description="Нажмите кнопку, чтобы заполнить анкету.", color=0x00ff88)
        await channel.send(embed=embed, view=AnketaView())
        await inter.response.send_message(f"✅ Кнопка отправлена в {channel.mention}", ephemeral=True)

    def cog_load(self):
        self.bot.add_view(AnketaView())
