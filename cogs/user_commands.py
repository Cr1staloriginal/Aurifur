...
class UserCommands(commands.Cog):
    """Пользовательские команды для фурри-РП и развлечений"""
    ...
    async def lick(self, inter: disnake.ApplicationCommandInteraction, member: disnake.Member):
        """Лизание как приветствие"""
        if member == inter.author:
            await inter.response.send_message(
                f"{inter.author.mention} пытается лизнуть себя, но не достаёт языком! 👅😂"
            )
        else:
            responses = [
    ...
    async def play(self, inter: disnake.ApplicationCommandInteraction, member: disnake.Member):
        ...
        if member == inter.author:
            await inter.response.send_message(
                f"{inter.author.mention} играет сам с собой... но это не так весело! 🎾"
            )
        else:
            responses = [
    ...
    async def fursona(self, inter: disnake.ApplicationCommandInteraction):
        """Генерирование случайной фурсоны"""
    ...
    @commands.slash_command(
        name="кнб", description="✊ Камень, ножницы, бумага (игра с ботом)"
    )
    async def rps(
        self,
        ...
        if выбор == bot_choice:
            ...
            (выбор == "камень" and bot_choice == "ножницы")
            or (выбор == "ножницы" and bot_choice == "бумага")
            or (выбор == "бумага" and bot_choice == "камень")
        ):
            result = "Ты выиграл!"
        else:
            result = "Я выиграл!"
    ...
    async def fursona_name(self, inter: disnake.ApplicationCommandInteraction):
        """Генерирование имени для фурсоны"""
